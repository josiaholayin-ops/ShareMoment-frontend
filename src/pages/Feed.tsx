...
function Avatar({ name, size = 44 }: { name: string; size?: number }) {
  return (
    <div className="rounded-full grid place-items-center ring-2 ring-white/20"
      style={{ width: size, height: size, background: 'radial-gradient(circle at 30% 30%, rgba(255,0,80,.9), rgba(0,242,234,.8))' }}>
...
export default function Feed() {
    ...
    const io = new IntersectionObserver(
      entries => {
        entries.forEach(entry => {
          ...
          if (entry.isIntersecting) { v.play().catch(() => {}) } else { v.pause(); v.currentTime = 0 }
        })
      }, { threshold: 0.7 }
    )
  ...
  const onTagClick = (tag: string) => setSearch(tag.replace(/^#/, ''))

  const ActionButton = ({ label, count, onClick, to }:{label:'like'|'comment'; count?:number; onClick?:()=>void; to?:string}) => {
    const isLike = label === 'like'
    const btn = (
      <button className="flex flex-col items-center gap-1 bg-black/40 hover:bg-black/60 backdrop-blur rounded-2xl px-3 py-2 border border-white/10 pointer-events-auto"
        onClick={onClick} aria-label={label} type="button">
        {isLike ? (
  ...
  return (
    <div className="max-w-3xl mx-auto px-3 py-4">
      <div className="flex items-center gap-2 mb-3">
        <input value={search} onChange={e => setSearch(e.target.value)} placeholder="Search by title, genre or tag..."
      ...
      <div ref={containerRef} className="space-y-10">
        {videos.map(v => {
          ...
          return (
            <div key={v.id} className="relative rounded-2xl overflow-hidden border border-white/10">
              <video src={`${apiBase}${v.filepath}`} muted playsInline loop className="w-full max-h-[80vh] object-cover bg-black" />
